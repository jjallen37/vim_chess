...
import { IKeyPressReader, ReaderCancelled } from './KeyPressReader';
...
export class Vim {
    ...
    /**
     * Stop the read loop (e.g. when the Vim mode toggle is switched off),
     * so a stale window listener doesn't swallow the next keypress
     */
    stop() {
        this._io.cancel();
    }
    ...
    async start() {
        ...
        do {
            ...
            var input;
            try {
                input = await this._io.read(isWhite);
            } catch (e) {
                if (e instanceof ReaderCancelled) { return; }
                throw e;
            }
    ...
    sXPawn = (state: IVimState) =>
    {
        var board = getBoard();
        var file = pawnToFile(state.inputKey);
        var isPlayer = board!!.isPlayersMove()
        var pawns = filter(board?.getPiecesSetup(), x => {
            return x.type === fromPiece(PIECE.P) 
    ...
    sQuickMove = (state: IVimState, validKeys: KEY[]) => {
        ...
        // var isPlayersTurn = getBoard()?.isPlayersMove()
    ...
    dispatchSelect(state: IVimState) {
        ...
        this._chess.markSquare(state.selected)
    ...
    dispatchDeselect(state: IVimState){
        ...
        state.arrowDirection = KEY.INVALID;
        this.unmarkTargetArrows(state)
        this._chess.unmarkSquare(state.selected)
    ...
    dispatchConfirm(state: IVimState){
        state.mode = Mode.S;
        this._chess.markSquare(state.target)
        this._chess.drawArrow(state.selected, state.target)
    }
