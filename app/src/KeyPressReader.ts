...
export interface IKeyPressReader {
    ...
    // Aborts a pending read() (if any) so a stale window listener
    // doesn't swallow the next keypress after Vim mode is toggled off
    cancel() : void;
}
...
export class ReaderCancelled extends Error {}

/**
 * Shared window-level, capture-phase keydown listener with cancellation.
 * Capture phase + document_start injection means this runs before
 * chess.com's own hotkey handlers get a chance to, regardless of which
 * key (e.g. "x", "f") they're also bound to.
 */
abstract class BaseKeyPressReader implements IKeyPressReader {
    private pendingListener: ((e: KeyboardEvent) => void) | null = null;
    private pendingReject: ((reason: Error) => void) | null = null;

    abstract read(isWhite: boolean) : Promise<IKeyPress>;

    protected readKey() : Promise<KeyboardEvent> {
    ...
    cancel() {
        if (this.pendingListener) {
            window.removeEventListener('keydown', this.pendingListener, {capture: true});
            this.pendingListener = null;
        }
        if (this.pendingReject) {
            const reject = this.pendingReject;
            this.pendingReject = null;
            reject(new ReaderCancelled());
        }
    }
}

export class KeyPressReader extends BaseKeyPressReader {
    read(isWhite: boolean) : Promise<IKeyPress> {
        return this.readKey().then(e => ({
...
export class InputKeypressReader extends BaseKeyPressReader {
...
function convertKeyDown(e: KeyboardEvent, isWhite: boolean) : KEY {
    ...
    // All other keystrokes destroyed here
        ...
        // any other listener chess.com registered on window/document itself,
        // not just ones on descendant elements
